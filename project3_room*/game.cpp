...
Game::Game(){
    ...
    vector<vector<Room*>> map (5,vector<Room*>(5,NULL));
    vector<vector<Room*>> copy_map (5,vector<Room*>(5,NULL));
    //map.begin();
    ...
    while (on == true){
        bool has_gold = false;
        copy(copy_map,map,5);
        Coordinates spawn_location = spawn(map,a1,5);
        build_cave(5,map, a1.get_player_location());
        call_precept(map, a1);
        
        while (run == true){
            wumbus = set_wumbus_location(map, 5);
            int encounter = move(map, 5, a1);
            if (encounter == 2){
                has_gold = true;
            }
        ...
        if (choice == 3){
            on = false;
            run = false;
            Game_destructor(map,copy_map, 5);
            //delete copy_map[0][0];
        }
        if (choice == 1){
            on = true;
...
void Game::build_cave(int size,vector<vector<Room*>> &map, Coordinates a1){
    //srand ( unsigned ( std::time(0) ) );
    //random_shuffle(map.begin(),map.end(), myrandom);
    cout << endl;
    for (int i = 0; i < size; i++){
        for (int p = 0; p < size; p++){
            room(i,p,size,map,a1);
        }
    }
...
void Game::room(int i, int p, int size,vector<vector<Room*>> &map, Coordinates a1){
    cout<<"*---*";
    if (p==size-1){
        cout<<endl;
        for(int z = 0; z < size; z++){
            ...
            if ((i == a1.x) && (z == a1.y)){
                ...
                icon = map[i][z]->get_event()->get_icon();
            }
            cout<<"| "<< icon <<" |";
            if (z==size-1){
                cout<<endl;
...
void Game::fill_events(vector<vector<Room*>> &map, int size){
   for (int i = 0; i < size; i++){
        for (int j = 0; j < size; j++){
            map[i][j]->set_event()(new Empty_room);
            map[i][j]->get_event()->set_coordinates(i,j);
        }
   }
   ...
   */
   ...
   for (int i = 0; i < size; i++){
        for (int j = 0; j < size; j++){
            cout <<map[i][j]->get_event()->get_icon()<< map[i][j]->get_event()->get_coordinates().x<<map[i][j]->get_event()->get_coordinates().y<< "coordinates"<<endl;
        }
   }
...
void Game::fill_events_two(vector<vector<Room*>> &map, int size){
    srand(time(NULL));
    for (int i = 0; i < 7;){
        int x = (rand() % (size-1));
        int y = (rand() % (size-1));
        if (map[x][y]->get_icon() == " "){
            if (i < 2){
                ...
                map[x][y]->set_event()(new Bottomless_pit);
            }
            if (i > 4){
                delete map[x][y]->get_event();
                map[x][y]->set_event()(new Super_bats);
            }
            if (i == 3){
                delete map[x][y]->get_event();
                map[x][y]->set_event()(new Gold);
            }
            if (i == 4){
                delete map[x][y]->get_event();
                map[x][y]->set_event()(new Wumbus);
            }
            i++;
        }
...
void Game::clear_events(vector<vector<Room*>> map, int size){
    for (int i = 0; i < size; i++){
        for (int j = 0; j < size; j++){
            map[i].pop_back();
        }
...
void Game::Game_destructor(vector<vector<Room*>> &map,vector<vector<Room*>> &copy_map, int size){
    for (int i = 0; i < size; i++){
        for (int j = 0; j < size; j++){
            delete map[i][j]->get_event();
            //delete copy_map[i][j];
        }
    }
    //delete map;
...
Coordinates Game::spawn(vector<vector<Room*>>&map ,Adventurer &a1, int size){
    srand(time(NULL));
    int x = (rand() % (size-1));
    int y = (rand() % (size-1));
    while(map[x][y]->get_event()->get_icon() != " "){
        x = (rand() % (size-1));
        y = (rand() % (size-1));
    }
...
int Game::move(vector<vector<Room*>> &map, int size,Adventurer &a1){
    ...
    int event = map[a1.get_player_location().x][a1.get_player_location().y]->get_event()->encounter(map,a1);
    //a1.set_player_location(x,y);
    build_cave(size, map, a1.get_player_location());
    return event;
    
}

void Game::shoot_arrow(char direction, Adventurer a1, vector<vector<Room*>> &map, int size){
...
int Game::check_wumbus(vector<vector<Room*>> &map, int x , int y, int rooms_flown_through){
    ...
    if (x > -1){
        if (x < size){
            if (y > -1){
                if (y < size){
                    icon = map[x][y]->get_event()->get_icon();
                }
            }
        }
    }
    ...
    if (icon == "w"){
        ...
        delete map[wumbus.x][wumbus.y]->get_event();
        map[wumbus.x][wumbus.y]->set_event()(new Empty_room);
        return 1;
    }
    else{
...
void Game::awake_wumbus(vector<vector<Room*>> &map){
    int c1 = (rand() % 100 + 1);
    int size;
    cout << c1;
    ...
    if (c1 < 75){
        ...
        delete map[x][y]->get_event();
        map[x][y]->set_event()(new Wumbus);
        delete map[tempx][tempy]->get_event();
        map[tempx][tempy]->set_event()(new Empty_room);
    }
}

Coordinates Game::set_wumbus_location(vector<vector<Room*>> &map, int size){
    //Coordinates wumbus;
    for (int x = 0; x < size; x++){
        for (int y = 0; y < size; y++)
            if (map[x][y]->get_event()->get_icon() == "w"){
                wumbus.x = x;
                wumbus.y = y;
            }
    }
...
void Game::call_precept(vector<vector<Room*>> &map,Adventurer &a1){
    int x;
    int y;
    int size = pow(map.size(), 0.5);
    ...
    if (x < (size-1)){
        map[x][y]->get_event()->percept();
        map[x+1][y]->percept();
    }
    if (x > 0){
        map[x-1][y]->get_event()->percept();
    }
        
    if (y < (size-1)){
        map[x][y+1]->get_event()->percept();
    }
    if (y > 0){
        map[x][y-1]->get_event()->percept();
    }
...
void Game::copy(vector<vector<Room*>> &base,vector<vector<Room*>> &blank, int size){
    for (int x = 0; x < size; x++){
        for (int y = 0; y < size; y++){
            blank[x][y]->get_event() = base[x][y]->get_event();
        }
    }
}
